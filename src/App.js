
import { Form ,Button, Navbar, Container,Nav, Row,Col,Image} from 'react-bootstrap';
import './App.css';
import avatar from './avatar.png'
function App() {
  const handlClick = ()=>{
    return alert('hello')
  }
  return (
    <div className="App">
      <Navbar bg="dark" variant="dark">
    <Container>
    <Navbar.Brand href="#home" style={{backgroundColor: "blue"}}>Navbar</Navbar.Brand>
    <Navbar className="me-auto">
      <Nav.Link href="#home">Home</Nav.Link>
      <Nav.Link href="#features">Features</Nav.Link>
      <Nav.Link href="#pricing">Pricing</Nav.Link>
    </Navbar>
    </Container>
  </Navbar>
  

    <Form>
  <Form.Group className="mb-3" controlId="formBasicEmail">
    <Form.Label>Email address</Form.Label>
    <Form.Control type="email" placeholder="Enter email" />
    <Form.Text className="text-muted">
      We'll never share your email with anyone else.
    </Form.Text>
  </Form.Group>

  <Form.Group className="mb-3" controlId="formBasicPassword">
    <Form.Label>Password</Form.Label>
    <Form.Control type="password" placeholder="Password" />
  </Form.Group>
  <Form.Group className="mb-3" controlId="formBasicCheckbox">
    <Form.Check type="checkbox" label="Check me out" />
  </Form.Group>
  <Button variant="primary" type="submit">
    Submit
  </Button>
</Form>
<Container>
  <Row>
    <Col xs={6} md={4}>
      <Image width="100px" height="100px" src="data:image/png;base64," rounded />
    </Col>
    <Col xs={6} md={4}>
      <Image width="100px" height="100px" src="https://cdn1.iconfinder.com/data/icons/avatar-97/32/avatar-02-512.png" roundedCircle />
    </Col>
    <Col xs={6} md={4}>
      <Image width="100px" height="100px" src="https://cdn.icon-icons.com/icons2/2468/PNG/512/user_kids_avatar_user_profile_icon_149314.png" thumbnail />
    </Col>
  </Row>
</Container>
  <img src='avv.png' alt="avatar"/>
  <img src={avatar} alt="avatar" onClick={()=>handlClick()}/>
    </div>
  );
}


export default App;
